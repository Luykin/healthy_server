...
import {modesInfo, setModesInfo} from "../../api";
...
export default class Model extends Component {
    ...
    //提交
    async _submit() {
        console.log(this.state.isOn, this.state.limit, this.state.isYuyue, this.state.startTime, this.state.endTime)
        const ret = await setModesInfo(this.state.isOn, this.state.limit, this.state.isYuyue, this.state.startTime, this.state.endTime);
        if (ret.code === 200) {
            Alert.alert("保存成功");
            NavigationUtil.goBack();
        } else {
            Alert.alert(ret.msg || "保存失败");
        }
    ...
    render() {
        ...
        return (
            <ScrollView style={[styles.container]} contentContainerStyle={{}}>
                ...
                <View style={[styles.view, {borderBottomWidth: 0.9, borderBottomColor: "#eee",}]}>
                ...
                <TouchableOpacity onPress={() => {
                    ...
                    <Text style={[comStyles.commonBtn, {marginTop: 60}]}>保存设置</Text>
                </TouchableOpacity>
            </ScrollView>
        )
    }
